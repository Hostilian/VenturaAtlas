...
| CHG-007 | Generators | Weak length/revision checks; tracked HEAD self-reference; comma-amplifying prose rewrite; timestamp churn | Complete content checks, atomic writes, content-derived revision, stable timestamps, idempotent prose synchronization | verified in clean clone; live writer correctly causes meta failure |
...
| CHG-013 | Tests | Headings implied provider HTTP failure injection | Contract names made truthful; public/source/security/cloud/UI/generator regressions added | 28/28 pass; provider HTTP injection still not performed |
...
| CHG-018 | Clean-clone proof | Lockfile initially failed `npm ci`; generators changed tracked files and ARCHITECTURE accumulated comma prefixes | Lockfile synchronized; fresh candidate clone installs, generates twice byte-stably, passes strict validation and full source-quality gate with zero diff | verified at `0a4c529` |
