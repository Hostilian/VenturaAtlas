#!/usr/bin/env node
/**
 * scripts/merge-gap-fill-ideas.js
 *
 * Merges data/gap-fill-ideas-ingest.json into data/ideas.json.
 * Idempotent: skips any idea whose ID already exists.
 *
 * Usage:
 *   node scripts/merge-gap-fill-ideas.js
 */
...
const INGEST_PATH = path.join(__dirname, '..', 'data', 'gap-fill-ideas-ingest.json');

console.log('Loading ideas.json...');
const ideasData = JSON.parse(fs.readFileSync(IDEAS_PATH, 'utf8'));

console.log('Loading gap-fill-ideas-ingest.json...');
